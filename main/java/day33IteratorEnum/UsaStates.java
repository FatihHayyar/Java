...
public enum UsaStates {
    ...
    UsaStates(String stateName, String abbreviation, String capital) {
...
public static String getStateNameFromAbbreviation(String abbreviation) {
   return Arrays.stream(values()).filter(t->t.getAbbreviation()==abbreviation).findFirst().get().getStateName();
}
public static String getStateNameFromCapital(String capital){
        return Arrays.stream(values()).filter(t->t.getCapital()==capital).findFirst().get().getStateName();
}
//baskenti verilen state in kisaltmasini ekrana yazdirin
public static String getAbbreviationFromCapital(String capital){
    return Arrays.stream(values()).filter(t->t.getCapital()==capital).findFirst().get().getAbbreviation();
}
}
